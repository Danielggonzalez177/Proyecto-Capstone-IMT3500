...
def iso_forest2(df, contamination, n_estimators=100, n_components=2, show=False):
...
def test_isolation(PATH_DICC_TEST,CONTAMINACION,FILENAME,LISTA_COLUMNA,N_ESTIMATORS=100, SHOW_PCA=False):
    df = pd.read_csv(PATH_DICC_TEST,delimiter=";")
    df = df[LISTA_COLUMNA].dropna()
    df_scores,df_outliers = iso_forest2(df, contamination=CONTAMINACION, n_estimators=N_ESTIMATORS,show=SHOW_PCA)
